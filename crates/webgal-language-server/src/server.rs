...
impl Backend {
    ...
    async fn change_file(&self, path: &str, kind: FileChangeKind) -> anyhow::Result<()> {
        ...
        let config_in_existing_project = {
            let workspace = self.workspace.read().await;
            config_in_existing_project(&workspace, path)
        };
        let is_config = name_of(path) == "config.txt";

        match kind {
            // 创建项目
            FileChangeKind::Create if is_config && !config_in_existing_project => {
                ...
                // 父项目优先, 丢弃此前已扫描到的全部子项目
                self.workspace.write().await.remove_all_under(project_path);
            ...
            // 移除项目
            FileChangeKind::Remove if is_config && !config_in_existing_project => {
                let project_path = parent_of(path);
                ...
                // 父项目移除后重新扫描, 恢复仍然存在的子项目
                Box::pin(self.initialize_workspace(project_path)).await?;
            }
...
fn config_in_existing_project(workspace: &Workspace, path: &str) -> bool {
    name_of(path) == "config.txt"
        && workspace
            .get(path)
            .is_some_and(|result| result.project_path != parent_of(path))
}
...
mod tests {
    ...
    fn insert_project(workspace: &mut Workspace, path: &str) {
        workspace.insert(path, Project::default()).unwrap();
    }

    #[test]
    fn config_route_distinguishes_project_and_resource() {
        let mut workspace = Workspace::new();
        insert_project(&mut workspace, "file:///C:/workspace/project");
        insert_project(&mut workspace, "file:///C:/workspace/child");

        assert!(!config_in_existing_project(
            &workspace,
            "file:///C:/workspace/new/config.txt"
        ));
        assert!(config_in_existing_project(
            &workspace,
            "file:///C:/workspace/project/child/config.txt"
        ));
        assert!(!config_in_existing_project(
            &workspace,
            "file:///C:/workspace/project/config.txt"
        ));
        assert!(!config_in_existing_project(
            &workspace,
            "file:///C:/workspace/project/scene/start.txt"
        ));
    }

    #[test]
    fn parent_project_replaces_discovered_child_projects() {
        let mut workspace = Workspace::new();
        insert_project(&mut workspace, "file:///C:/workspace/child");

        workspace.remove_all_under("file:///C:/workspace");
        insert_project(&mut workspace, "file:///C:/workspace");

        let result = workspace
            .get("file:///C:/workspace/child/scene/start.txt")
            .unwrap();
        assert_eq!(result.project_path, "file:///C:/workspace");
        assert_eq!(result.resource_path, "child/scene/start.txt");
    }

    #[test]
    fn removing_parent_allows_child_project_to_be_discovered_again() {
        let mut workspace = Workspace::new();
        insert_project(&mut workspace, "file:///C:/workspace");

        workspace.remove("file:///C:/workspace").unwrap();
        insert_project(&mut workspace, "file:///C:/workspace/child");

        let result = workspace
            .get("file:///C:/workspace/child/scene/start.txt")
            .unwrap();
        assert_eq!(result.project_path, "file:///C:/workspace/child");
