...
mod tests {
    ...
    fn roundtrip() {
        ...
        for (i, (original, expected)) in original.lines().zip(expected.lines()).enumerate() {
            ...
            let comment = split_once_escaped(original, ';')
                .unwrap_or_default()
                .1
                .trim();
    ...
    fn choose_sentence_multiple_options() {
        ...
        match output.sentence {
            Sentence::Choose(choose) => {
                let expected = vec![
                    Choice {
                        prompt: "Option A".to_string(),
                        target: Some("scene_a".to_string()),
                        ..Default::default()
                    },
                    Choice {
                        prompt: "Option B".to_string(),
                        target: Some("scene_b".to_string()),
                        ..Default::default()
                    },
                    Choice {
                        prompt: "Option C".to_string(),
                        target: Some("scene_c".to_string()),
                        ..Default::default()
                    },
                ];
                assert_eq!(choose.choices, expected);
    ...
    #[test]
    fn choose_with_conditions() {
        // 带条件选项
        let s = "choose:(show)[enable]->go:scene_a|(hide)[disabled]->stay:scene_b;";
        let output = Sentence::from_str(s);
        assert!(output.errors.is_empty());
        match output.sentence {
            Sentence::Choose(choose) => {
