...
`webgal-language-server` 提供了对 WebGAL 脚本丰富的自动补全功能。
...
下面简单介绍处于不同位置输入时 `webgal-language-server` 的补全方案。
...
2. **展开语句模板**  
   `webgal-language-server` 内置了一些方便的语句模板，通常以 `command.feature` 的形式显示。  
   选择模板后，将展开模板对应的补全。
...
主参数 / 参数值是 `webgal-language-server` 的一大亮点，具体请看[类型](#类型)章节。

## 类型

对于不同类型的主参数和参数值，`webgal-language-server` 也提供了相关的补全功能。
