...
";
    ...
    fn reported_line_matches_the_file_line_it_was_parsed_from() {
        /*
         * 行号错位会让"光标点中的行 / 高亮的行 / 面板显示的对话"三者互相错开,
         * 因此这里直接**回读原文本校验**: 对每一条对话, 用它报出的 `line` 去原文里取
         * 那一行, 必须能解析出同一条对话。
         *
         * 场景刻意包含空行与各种非对话语句 —— 它们最容易让行号偏移一位。
         */
        let scene = r"
...
";
        ...
        // 逐行核对 (0 起): 空行 / 资源语句 / 控制语句都不应出现在结果里
        assert_eq!(
        ...
        assert_eq!(lines[0].text, "你爱我吗？");
        assert_eq!(lines[1].text, "唉？");
        // `next:对话;` 是显式说话者, 内容为「对话」
        assert_eq!(lines[2].text, "对话");
        // 显式旁白 (`:内容;`) 不继承前面的说话者
        assert_eq!(lines[3].speaker, None);
        assert!(!lines[3].speaker_inherited);
