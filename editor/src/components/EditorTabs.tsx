...
// 多选项卡栏: 每个打开的场景/文件、文本预处理、差异比较、配音帮助各对应一个选项卡。
// 支持拖拽调整顺序 (基于 Pointer 事件: HTML5 DnD 在部分 WebView 中不可靠)。
//
// 场景选项卡在**选中时**显示一个麦克风按钮: 带斜杠表示未启动配音模式,
// 点击后在「普通编辑」与「配音模式」之间切换。未选中时不显示, 以免干扰。
...
export function EditorTabs() {
  ...
  const helpTabs = useAppStore((s) => s.helpTabs);
  const activeHelpId = useAppStore((s) => s.activeHelpId);
  const setActiveHelp = useAppStore((s) => s.setActiveHelp);
  const closeHelpTab = useAppStore((s) => s.closeHelpTab);
  ...
  if (documents.length === 0 && novelTabs.length === 0 && diffTabs.length === 0 && helpTabs.length === 0) return null;
    ...
    <div className="editor-tabs" role="tablist" ref={containerRef}>
      {documents.map((d, index) => {
        ...
        return (
          ...
          >
            ...
            {/* 配音模式指示与切换: 配音卡恒显示, 普通场景卡仅选中时显示 */}
            {(inVoiceMode || d.path === activePath) && (
              <Tooltip content={inVoiceMode ? '退出配音模式' : '进入配音模式'} relationship="label">
                <button
      ...
      {helpTabs.map((t) => (
        <div
          key={t.id}
          role="tab"
          aria-selected={t.id === activeHelpId}
          className={`editor-tab help${t.id === activeHelpId ? ' active' : ''}`}
          onClick={() => setActiveHelp(t.id)}
          title={t.title}
        >
          <span className="editor-tab-name">
            <span className="editor-tab-icon">
